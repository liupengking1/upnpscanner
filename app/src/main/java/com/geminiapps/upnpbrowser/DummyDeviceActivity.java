 ...
 */
// DOC:CLASS
public class DummyDeviceActivity extends Activity implements PropertyChangeListener {
    ...
    public static String mDeviceNmae = "";
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            LocalService<DummyService> switchPowerService = getSwitchPowerService();

            // Register the device when this activity binds to the service for the first time
            if (switchPowerService == null) {
                try {
                    LocalDevice dummyDevice = createDevice();

                    Toast.makeText(DummyDeviceActivity.this, R.string.registeringDevice, Toast.LENGTH_SHORT).show();
                    upnpService.getRegistry().addDevice(dummyDevice);

                    switchPowerService = getSwitchPowerService();

                } catch (Exception ex) {
                    Timber.e("Creating dummy device failed,%s", ex);
                    Toast.makeText(DummyDeviceActivity.this, R.string.createDeviceFailed, Toast.LENGTH_SHORT).show();
                    return;
                }
            }
            ...
            switchPowerService.getManager().getImplementation().getPropertyChangeSupport()
                    .addPropertyChangeListener(DummyDeviceActivity.this);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDeviceNmae = getIntent().getStringExtra("deviceName");
    ...
    protected LocalService<DummyService> getSwitchPowerService() {
        if (upnpService == null)
            return null;

        LocalDevice dummyDevice;
        if ((dummyDevice = upnpService.getRegistry().getLocalDevice(udn, true)) == null)
            return null;

        return (LocalService<DummyService>)
                dummyDevice.findService(new UDAServiceType("DummyService", 1));
    }
    ...
    public void propertyChange(PropertyChangeEvent event) {
        ...
        if (event.getPropertyName().equals("status")) {
            Timber.i("Turning light: " + event.getNewValue());
    ...
    protected LocalDevice createDevice()
        ...
        DeviceType type =
                new UDADeviceType(mDeviceType, 1);

        DeviceDetails details =
                new DeviceDetails(
                        mDeviceNmae,
                        new ManufacturerDetails("GeminiApps"),
                        new ModelDetails("Dummy Device", "Dummy service", "v1")
                );

        LocalService service =
                new AnnotationLocalServiceBinder().read(DummyService.class);

        service.setManager(
                new DefaultServiceManager<>(service, DummyService.class)
        );
